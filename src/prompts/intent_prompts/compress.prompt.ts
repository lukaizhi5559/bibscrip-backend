 ...
 * Purpose: Compress files or folders
 * Available Actions: compressFile, fileExists, screenshot, end
 */

import { IntentExecutionRequest } from '../../types/intentTypes';

export function buildCompressPrompt(request: IntentExecutionRequest, actionHistory?: any[]): string {
  const { stepData, context } = request;
  
  return `You are executing a COMPRESS intent. Your goal: ${stepData.description}
...
=== AVAILABLE ACTIONS ===
compressFile, fileExists, screenshot, end

=== CRITICAL RULES ===
Creates .zip archive. Specify source and destination.
